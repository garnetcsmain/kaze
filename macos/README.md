...
**Implementation drafting (built):** the moment a behavior crosses the 3-day confirmation threshold, a second, dedicated LLM call drafts a concrete, ready-to-use fix for it — not just the one-line suggestion, but the actual artifact: a shell alias/function, the exact steps for a macOS Shortcut, an editor snippet, a browser bookmarklet, a launchd/cron job, or a specific process change when nothing is toolable. This is intentionally **draft-only**: Kaze never runs, installs, or applies anything itself — the draft (plus a risk caveat when relevant) shows up as a copyable block in the Insights window and in the Obsidian journal, and it's entirely up to you whether and how to adopt it. Ledger entries confirmed before this existed can be drafted retroactively via the "Draft implementation" button in Insights.

**Outcome tracking (built):** every suggestion card asks for a decision — **"I implemented this"** or **"Ignore"** (with an optional why). Both feed back into every future analysis prompt: ignored suggestions are never repeated (the model must respect the stated reason, and may only re-report the behavior with a materially different fix), and adopted ones are **watched for recurrence** — the ledger keeps merging daily, so if a behavior is seen again after its fix was adopted (past a 36h grace window, since the morning analysis covers the adoption day itself), the fix is flagged as *not holding* in Insights, the daily journal note ("Fix outcomes"), and the morning notification. This closes the loop on whether recommendations were actually good enough, not just whether they were made. Decisions are stored on the observation (`resolution` / `resolutionReason` / `resolutionAt`) and reversible via "Reopen".
